...
opts = Optimist::options do
  opt :start, 'starting github issue to avoid rate limiting', type: :int
  opt :stop, 'stopping github issue to avoid rate limiting', type: :int
  opt :renumber, 'start the issues at this number to avoid overwriting existing issues', type: :int
end
...
def link_to_github(github_resource)
  "\nImported from github: #{github_resource.html_url} at #{Time.now}"
end
...
issues.each do |issue|
  ...
  target_issue_number =  opts[:renumber] ? issue.number + opts[:renumber] : issue.number

  issue_jira = {
    "key" => "#{jirakey}-#{target_issue_number}",
  ...
  issue_jira["description"].concat(link_to_github(issue))
  ...
  issue_comments.each do |comment|
    ...
    jira_comment["body"].concat(link_to_github(comment))
    issue_jira['comments'] << jira_comment
  end
  ...
  if pr
    ...
    jira_pr["body"].concat(link_to_github(pr_info))
    issue_jira['comments'] << jira_pr
  end
